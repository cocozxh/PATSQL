...
public class WindowPrune implements FillingStrategy {
	...
	public List<RAOperator> fill(RAOperator sketch, Example example, SynthOption option) {
		...
		List<WinColSchema> winSchemas = all(tmpTable);
		// List<WinColSchema> winSchemas = allInferByWinFuncandColName(tmpTable, outTable);
	...
	private List<WinColSchema> allInferByWinFuncandColName(Table tmpTable, Table outTable){
		List<WinColSchema> winSchemas = new ArrayList<>();
		Map<String, List<ColSchema>> srcColName2ColSchema = tmpTable.getSrcColName2ColSchemaMap();
	
		for(ColSchema outputCol:outTable.schema()){
			if(outputCol.winFuncs.size()==0) continue;

			for(WinFunc func:outputCol.winFuncs){
				List<ColSchema> selectedSchemas = new ArrayList<>();

				if(srcColName2ColSchema.containsKey(outputCol.getSrcColName())){
					selectedSchemas = srcColName2ColSchema.get(outputCol.getSrcColName());
					//rank() has no params
					selectedSchemas.add(null);
				}
				else{
					selectedSchemas = srcColumns(tmpTable);
				}

				for(ColSchema src:selectedSchemas){
					for (GroupKeys pKey : allpartitionKey(tmpTable)) {
						for (SortKey oKey : orderbyKey(tmpTable)) {
							WinColSchema wsc = new WinColSchema(func, src, pKey, oKey);
							if (wsc.isValid()) {
	...
	private List<ColSchema> srcColumns(Table table) {
	...
	private GroupKeys[] allpartitionKey(Table table) {
