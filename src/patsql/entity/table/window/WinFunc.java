...
public enum WinFunc {
	...
	// get agg by agg name (no matter it's lowercase or uppercase)
	public static WinFunc findWinFuncByName(String name){
		WinFunc result = null;
		for(WinFunc winFunc:values()){
			if(winFunc.name().equalsIgnoreCase(name)){
				result = winFunc;
				break;
			}
		}
